...
AccInv ==
  \A a \in Acceptors:
    ...
    \* conjunct added corresponding to MsgInv 2014/03/29 sms
...
<1>2. Inv /\ [Next]_vars => Inv'
  <2> SUFFICES ASSUME Inv, Next
               PROVE  Inv'
    BY Z3T(30) DEF vars, Inv, TypeOK, MsgInv, AccInv, SafeAt, VotedForIn, WontVoteIn
  <2> USE DEF Inv
  <2>1. TypeOK'
      ...
      <4>10. SafeAt(v,b)
        ...
        <5>2. CASE b # 0
            ...
            <6>3. CASE \A m \in S : m.maxVBal = -1
                ...
                <7>4. \A c \in 0 .. (b-1) : \A QQ \in Quorums(c) :
                        Q \cap QQ # {}
                    BY QuorumAssumption
                <7>5. \A c \in 0 .. (b-1), vv \in Values : ~ChosenIn(c,vv)
                    BY <7>4 DEF ChosenIn
...
\* Last modified Wed Jan 27 14:59:22 CET 2021 by alexis51151
